/*
Dla struktury Fig napisz funkcję fn save(filename:&str,v:Vec<Fig>)pozwalającą
na zapis wektora Figur do pliku, oraz funkcję fn load(filename:&str)-> Vec<Fig>),
pozwalającą na odtworzenie zawartości wektora figur z pliku. Po wykonaniu kodu:
...
fn save(filename: &str, v: Vec<Fig>) {
    let mut file = File::create(filename).expect("Nie można utworzyć pliku");
    for fig in v {
        let line = match fig {
            Koło { r } => format!("Koło {}\n", r),
        ...
        file.write_all(line.as_bytes()).expect("Błąd zapisu");
    }
}

fn load(filename: &str) -> Vec<Fig> {
